...
func execCommand(inputPayload string) ([]byte, error) {
	...
	// datadog.yaml.
	log.Printf("%s | calling secret_backend_command with payload: '%s'", time.Now().String(), inputPayload)
	start := time.Now()
	err := cmd.Run()
	elapsed := time.Since(start)
	log.Printf("%s | secret_backend_command '%s' completed in %s", time.Now().String(), secretBackendCommand, elapsed)
	...
	if err != nil {
		log.Printf("secret_backend_command stderr: %s", stderr.buf.String())
