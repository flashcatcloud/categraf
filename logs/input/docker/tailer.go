...
func NewTailer(cli *dockerutil.DockerUtil, containerID string, source *logsconfig.LogSource, outputChan chan *message.Message, erroredContainerID chan string, readTimeout time.Duration) *Tailer {
	return &Tailer{
...
func (t *Tailer) Stop() {
	log.Printf("Stop tailing container: %v", ShortContainerID(t.ContainerID))
	t.stop <- struct{}{}
...
func (t *Tailer) Start(since time.Time) error {
	log.Printf("Start tailing container: %v", ShortContainerID(t.ContainerID))
	return t.tail(since.Format(logsconfig.DateFormat))
}

func (t *Tailer) getLastSince() string {
	t.mutex.Lock()
	defer t.mutex.Unlock()
	since, err := time.Parse(logsconfig.DateFormat, t.lastSince)
	if err != nil {
		since = time.Now().UTC()
	} else {
	...
	return since.Format(logsconfig.DateFormat)
}
...
func (t *Tailer) tryRestartReader(reason string) error {
	log.Printf("%s for container %v", reason, ShortContainerID(t.ContainerID))
	t.wait()
	err := t.setupReader()
	if err != nil {
		log.Printf("Could not restart the docker reader for container %v: %v:", ShortContainerID(t.ContainerID), err)
	...
	for {
		...
		default:
			...
			if err != nil { // an error occurred, stop from reading new logs
				...
				default:
					t.source.Status.Error(err)
					log.Printf("Could not tail logs for container %v: %v", ShortContainerID(t.ContainerID), err)
					t.erroredContainerID <- t.ContainerID
					return
				}
			}
