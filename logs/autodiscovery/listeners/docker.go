...
func (l *DockerListener) Listen(newSvc chan<- Service, delSvc chan<- Service) {
	...
	if err != nil {
		log.Printf("can't listen to docker events: %v", err)
		return
	}
		...
		for {
			...
			case err := <-errs:
				if err != nil && err != io.EOF {
					log.Printf("docker listener error: %v", err)
				}
				cancel()
				return
			}
...
func (l *DockerListener) init() {
	...
	if err != nil {
		log.Printf("Couldn't retrieve container list - %s", err)
	}
...
func (l *DockerListener) processEvent(ctx context.Context, e *docker.ContainerEvent) {
	...
	if found {
		...
		default:
			...
			log.Printf("Expected die for container %s got %s: skipping event", cID[:12], e.Action)
			return
		}
	} else {
...
func (l *DockerListener) createService(ctx context.Context, cID string) {
	...
	if err != nil {
		log.Printf("Failed to inspect container '%s', not creating AD service, err: %s", cID[:12], err)
		return
	}

	containerImage, err = l.dockerUtil.ResolveImageNameFromContainer(ctx, cInspect)
	if err != nil {
		log.Printf("error while resolving image name: %s", err)
		containerImage = ""
	}
	...
	if l.filters.IsExcluded(containers.GlobalFilter, containerName, containerImage, "") {
		log.Printf("container %s filtered out: name %q image %q", cID[:12], containerName, containerImage)
		return
	}
	...
	if !cInspect.State.Running && cInspect.State.FinishedAt != "" {
		...
		if err == nil {
			excludeAge := time.Duration(coreconfig.GetContainerExcludeStoppedAge()) * time.Hour
			if time.Now().Sub(finishedAt) > excludeAge {
				log.Printf("container %q not running for too long, skipping", cID[:12])
				return
			}
		} else {
			log.Printf("container %q not running, but can't parse FinishedAt: %s", cID[:12])
		}
	}
	...
	if err != nil {
		log.Printf("Error getting check names from docker labels on container %s: %v", cID, err)
	}
	...
	if err != nil {
		log.Printf("Failed to inspect container %s - %s", cID[:12], err)
	}
...
func (l *DockerListener) removeService(cID string) {
	...
	if ok {
		...
		log.Printf("Container %s not found, not removing", cID[:12])
	}
}
...
func (s *DockerService) GetPorts(ctx context.Context) ([]ContainerPort, error) {
	...
	case cInspect.Config != nil && len(cInspect.Config.ExposedPorts) > 0:
		log.Printf("using ExposedPorts for container %s as no port bindings are listed", s.cID[:12])
