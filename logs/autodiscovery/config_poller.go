...
func (pd *configPoller) poll(ac *AutoConfig) {
	...
	for {
		...
		case <-pd.stopChan:
			...
			return
		case <-ticker.C:
			log.Printf("Polling %s config provider", pd.provider.String())
			...
			if err != nil {
				log.Printf("Cache processing of %v configuration provider failed: %v", pd.provider, err)
			}
			if upToDate {
				log.Printf("No modifications in the templates stored in %v configuration provider", pd.provider)
				break
			}
			...
			if len(newConfigs) > 0 || len(removedConfigs) > 0 {
				log.Printf("%v provider: collected %d new configurations, removed %d", pd.provider, len(newConfigs), len(removedConfigs))
			} else {
				log.Printf("%v provider: no configuration change", pd.provider)
			}
...
func (pd *configPoller) collect(ctx context.Context) ([]integration.Config, []integration.Config) {
	...
	if err != nil {
		log.Printf("Unable to collect configurations from provider %s: %s", pd.provider, err)
		return nil, nil
	}
