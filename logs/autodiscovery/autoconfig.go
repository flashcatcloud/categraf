...
func init() {
	acErrors = expvar.NewMap("autodiscovery")
...
func (ac *AutoConfig) checkTagFreshness(ctx context.Context) {
	...
	for _, service := range servicesToRefresh {
		log.Printf("Tags changed for service %s, rescheduling associated checks if any", service.GetTaggerEntity())
...
func (ac *AutoConfig) Stop() {
	...
	// stop the listeners retry logic if running
...
func (ac *AutoConfig) AddConfigProvider(provider providers.ConfigProvider, shouldPoll bool, pollInterval time.Duration) {
	...
	for _, pd := range ac.providers {
		if pd.provider == provider {
			...
			log.Printf("Provider %s was already added, skipping...", provider)
			return
		}
	}
...
func (ac *AutoConfig) addListenerCandidates(listenerConfigs []Listeners) {
	...
	for _, c := range listenerConfigs {
		...
		if !ok {
			...
			log.Printf("Listener %s was not registered", c.Name)
			continue
		}
		log.Printf("Listener %s was registered", c.Name)
...
func (ac *AutoConfig) initListenerCandidates() bool {
	...
	for name, factory := range ac.listenerCandidates {
		...
		case err == nil:
			...
			log.Printf("%s listeners successfully started", name)
		...
		case retry.IsErrWillRetry(err):
			...
			log.Printf("%s listeners cannot start, will retry: %s", name, err)
		default:
			// Log an error and remove from candidates
			log.Printf("%s listeners cannot start: %s", name, err)
			delete(ac.listenerCandidates, name)
		}
	}
