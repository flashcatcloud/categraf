...
import (
	...
	"flashcat.cloud/categraf/logs/autodiscovery/providers/names"
...
func (s *Scheduler) Schedule(configs []integration.Config) {
	for _, config := range configs {
		...
		if config.HasFilter(containers.LogsFilter) {
			log.Printf("Config %s is filtered out for logs collection, ignoring it", s.configName(config))
			continue
		}
		switch {
		case s.newSources(config):
			log.Printf("Received a new logs config: %v", s.configName(config))
		...
		case s.newService(config):
			...
			if err != nil {
				log.Printf("Invalid service: %v", err)
				continue
			}
			...
			log.Printf("Received a new service: %v", config.Entity)
...
func (s *Scheduler) Unschedule(configs []integration.Config) {
	for _, config := range configs {
		...
		switch {
		case s.newSources(config):
			log.Printf("New source to remove: entity: %v", config.Entity)
		...
		case s.newService(config):
			...
			log.Printf("New service to remove: entity: %v", config.Entity)
			service, err := s.toService(config)
			if err != nil {
				log.Printf("Invalid service: %v", err)
				continue
...
func (s *Scheduler) toSources(config integration.Config) ([]*logsConfig.LogSource, error) {
	...
	for _, cfg := range configs {
		...
		if err := cfg.Validate(); err != nil {
			log.Printf("Invalid logs configuration: %v", err)
