...
func (t *Task) doneBefore() bool {
	doneFlag := path.Join(config.Config.Ibex.MetaDir, fmt.Sprint(t.Id), fmt.Sprintf("%d.done", t.Clock))
...
func (t *Task) prepare() error {
	...
	IdDir := path.Join(config.Config.Ibex.MetaDir, fmt.Sprint(t.Id))
...
func (t *Task) start() {
	...
	scriptFile := path.Join(config.Config.Ibex.MetaDir, fmt.Sprint(t.Id), "script")
