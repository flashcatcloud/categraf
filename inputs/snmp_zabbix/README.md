...
- 步骤2: 导航到模板页面
配置(Data Collection) -> 模板(Templates)
...
template_files = [
    ...
    "/etc/categraf/templates/interface_addon.yaml"  # 多个模板会合并
]
```
方式二：内嵌模板内容(用于配置下发)
```toml
...
#### 4.2.1 发现功能配置
```toml
# template不为空,则自动启用自动发现

# 发现功能会自动从模板中读取 discovery_rules
# 无需额外配置
```
发现调度机制：
- 每个发现规则按其 delay 独立调度
...
```toml
# 批量采集设置（内部优化，不可配置）
# 插件自动将 OID 分批，每批最多 60 个

# 并发控制
# 插件对每个 agent 使用独立的锁，避免并发冲突
