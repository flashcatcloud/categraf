...
func NewILM(u *url.URL, hc *http.Client) (Collector, error) {
	return &ILM{
		hc: hc,
		u:  u,
	}, nil
...
func (i *ILM) Update(ctx context.Context, ch chan<- prometheus.Metric) error {
	...
	indexResp, err := getURL(ctx, i.hc, indexURL.String())
	if err != nil {
	...
	indexStatusResp, err := getURL(ctx, i.hc, indexStatusURL.String())
	if err != nil {
		return fmt.Errorf("failed to load ILM url: %w", err)
	}
