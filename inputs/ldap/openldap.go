...
func (ins *Instance) newOpenLDAPConfig() []request {
	...
	return []request{{req, ins.convertOpenLDAP}}
}

func (ins *Instance) convertOpenLDAP(result *ldap.SearchResult, ts time.Time) []types.Metric {
	...
	for _, entry := range result.Entries {
		prefix := openLDAPAttrConvertDN(entry.DN, ins.ReverseFieldNames)
