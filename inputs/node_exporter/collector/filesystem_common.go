...
var (
	...
	*/
	oldMountPointsExcluded = new(string)
	/* = kingpin.Flag(
...
func filesystemCollectorInit(params map[string]string) {
	mpe, ok := params["collector.filesystem.mount-points-exclude"]
	if !ok {
		*mountPointsExclude = defMountPointsExcluded
	} else {
		mountPointsExcludeSet = true
		*mountPointsExclude = mpe
	}
	*oldMountPointsExcluded = params["collector.filesystem.ignored-mount-points"]

	fte, ok := params["collector.filesystem.fs-types-exclude"]
	if !ok {
	...
	*oldFSTypesExcluded = params["collector.filesystem.ignored-fs-types"]
}
