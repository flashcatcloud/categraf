...
var (
	netdevDeviceInclude    = new(string) // kingpin.Flag("collector.netdev.device-include", "Regexp of net devices to include (mutually exclusive to device-exclude).").String()
	oldNetdevDeviceInclude = new(string) // kingpin.Flag("collector.netdev.device-whitelist", "DEPRECATED: Use collector.netdev.device-include").Hidden().String()
	netdevDeviceExclude    = new(string) // kingpin.Flag("collector.netdev.device-exclude", "Regexp of net devices to exclude (mutually exclusive to device-include).").String()
	oldNetdevDeviceExclude = new(string) // kingpin.Flag("collector.netdev.device-blacklist", "DEPRECATED: Use collector.netdev.device-exclude").Hidden().String()
	netdevAddressInfo      = new(bool)   // kingpin.Flag("collector.netdev.address-info", "Collect address-info for every device").Bool()
	netdevDetailedMetrics  = new(bool)   // kingpin.Flag("collector.netdev.enable-detailed-metrics", "Use (incompatible) metric names that provide more detailed stats on Linux").Bool()
)
...
func netDevCollectorInit(params map[string]string) {
	*netdevDeviceInclude = params["collector.netdev.device-include"]
	*oldNetdevDeviceInclude = params["collector.netdev.device-whitelist"]
	*netdevDeviceExclude = params["collector.netdev.device-exclude"]
	*oldNetdevDeviceExclude = params["collector.netdev.device-blacklist"]
	*netdevAddressInfo = params["collector.netdev.address-info"] == "true"
	*netdevDetailedMetrics = params["collector.netdev.enable-detailed-metrics"] == "true"

	// cnn, ok := params["collector.netdev.netlink"]
	// if !ok {
	// 	*netDevNetlink = true
	// } else {
	// 	*netDevNetlink = cnn == "true"
	// }
	// swc, ok := params["collector.filesystem.stat-workers"]
	// if !ok {
	// 	*statWorkerCount = 4
	// } else {
	// 	cnt, err := strconv.Atoi(swc)
	// 	if err == nil {
	// 		*statWorkerCount = cnt
	// 	}
	// }
	// mt, ok := params["collector.filesystem.mount-timeout"]
	// if !ok {
	// 	*mountTimeout = 5 * time.Second
	// } else {
	// 	tm, err := time.ParseDuration(mt)
	// 	if err != nil {
	// 		*mountTimeout = tm
	// 	}
	// }
