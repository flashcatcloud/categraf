...
var mountTimeout = new(time.Duration)

/*
	kingpin.Flag("collector.filesystem.mount-timeout",

"how long to wait for a mount to respond before marking it as stale").
Hidden().Default("5s").Duration()
*/
var statWorkerCount = new(int)

/*
	kingpin.Flag("collector.filesystem.stat-workers",
