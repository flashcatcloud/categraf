...
var (
	ignoredTapeDevices = new(string)
	// kingpin.Flag("collector.tapestats.ignored-devices", "Regexp of devices to ignore for tapestats.").Default("^$").String()
)
...
func NewTapestatsCollector() (Collector, error) {
...
func tapestatsCollectorInit(params map[string]string) {
	itd, ok := params["collector.tapestats.ignored-devices"]
	if !ok {
		*ignoredTapeDevices = "^$"
	} else {
		*ignoredTapeDevices = itd
	}
}
