...
var statSoftirqFlag = new(bool) // kingpin.Flag("collector.stat.softirq", "Export softirq calls per vector").Default("false").Bool()
...
func statCollectorInit(params map[string]string) {
	*statSoftirqFlag = params["collector.stat.softirq"] == "true"
}
