...
var (
	...
	ipvsLabels = new(string)
	// kingpin.Flag("collector.ipvs.backend-labels", "Comma separated list for IPVS backend stats labels.").Default(strings.Join(fullIpvsBackendLabels, ",")).String()
)
...
func ipvsCollectorInit(params map[string]string) {

	ivl, ok := params["collector.ipvs.backend-labels"]
	if !ok {
		*ipvsLabels = strings.Join(fullIpvsBackendLabels, ",")
	} else {
		*ipvsLabels = ivl
	}
}
