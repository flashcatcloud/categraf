...
var (
	ethtoolDeviceInclude   = new(string) // = kingpin.Flag("collector.ethtool.device-include", "Regexp of ethtool devices to include (mutually exclusive to device-exclude).").String()
	ethtoolDeviceExclude   = new(string) // = kingpin.Flag("collector.ethtool.device-exclude", "Regexp of ethtool devices to exclude (mutually exclusive to device-include).").String()
	ethtoolIncludedMetrics = new(string) // = kingpin.Flag("collector.ethtool.metrics-include", "Regexp of ethtool stats to include.").Default(".*").String()
...
// for testing.
func makeEthtoolCollector() (*ethtoolCollector, error) {
...
func NewEthtoolCollector() (Collector, error) {
	return makeEthtoolCollector()
}
...
func ethtoolCollectorInit(params map[string]string) {
	*ethtoolDeviceInclude = params["collector.ethtool.device-include"]
	*ethtoolDeviceExclude = params["collector.ethtool.device-exclude"]
	includeMetrics, ok := params["collector.ethtool.metrics-include"]
	if !ok {
		*ethtoolIncludedMetrics = ".*"
	} else {
		*ethtoolIncludedMetrics = includeMetrics
	}
}
...
func (c *ethtoolCollector) Update(ch chan<- prometheus.Metric) error {
	...
	for device := range netClass {
		...
		for metric := range stats {
			...
			_, exists := metricFQNames[metricFQName]
			if exists {
