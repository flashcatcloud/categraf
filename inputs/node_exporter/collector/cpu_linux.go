...
var (
	enableCPUGuest       = new(bool)   //  = kingpin.Flag("collector.cpu.guest", "Enables metric node_cpu_guest_seconds_total").Default("true").Bool()
	enableCPUInfo        = new(bool)   // = kingpin.Flag("collector.cpu.info", "Enables metric cpu_info").Bool()
	flagsInclude         = new(string) //       = kingpin.Flag("collector.cpu.info.flags-include", "Filter the `flags` field in cpuInfo with a value that must be a regular expression").String()
	bugsInclude          = new(string) // = kingpin.Flag("collector.cpu.info.bugs-include", "Filter the `bugs` field in cpuInfo with a value that must be a regular expression").String()
...
func cpuCollectorInit(params map[string]string) {
	ecg, ok := params["collector.cpu.guest"]
	if !ok {
		*enableCPUGuest = true
	} else {
		*enableCPUGuest = ecg == "true"
	}

	eci := params["collector.cpu.info"]
	*enableCPUInfo = eci == "true"

	*flagsInclude = params["collector.cpu.info.flags-include"]
	*bugsInclude = params["collector.cpu.info.bugs-include"]
}
