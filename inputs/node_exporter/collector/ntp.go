...
var (
	ntpServer          = new(string) // kingpin.Flag("collector.ntp.server", "NTP server to use for ntp collector").Default("127.0.0.1").String()
	ntpServerPort      = new(int)    // kingpin.Flag("collector.ntp.server-port", "UDP port number to connect to on NTP server").Default("123").Int()
	ntpProtocolVersion = new(int)    // kingpin.Flag("collector.ntp.protocol-version", "NTP protocol version").Default("4").Int()
	ntpServerIsLocal   = new(bool)   // kingpin.Flag("collector.ntp.server-is-local", "Certify that collector.ntp.server address is not a public ntp server").Default("false").Bool()
	ntpIPTTL           = new(int)    // kingpin.Flag("collector.ntp.ip-ttl", "IP TTL to use while sending NTP query").Default("1").Int()
	...
	ntpMaxDistance     = new(time.Duration) // kingpin.Flag("collector.ntp.max-distance", "Max accumulated distance to the root").Default("3.46608s").Duration()
	ntpOffsetTolerance = new(time.Duration) // kingpin.Flag("collector.ntp.local-offset-tolerance", "Offset between local clock and local ntpd time to tolerate").Default("1ms").Duration()
...
func ntpCollectorInit(params map[string]string) {
	ns, ok := params["collector.ntp.server"]
	if !ok {
		*ntpServer = "127.0.0.1"
	} else {
		*ntpServer = ns
	}
	nsp, ok := params["collector.ntp.server-port"]
	if !ok {
		*ntpServerPort = 123
	} else {
		port, err := strconv.Atoi(nsp)
		if err == nil {
			*ntpServerPort = port
		}
	}
	nv, ok := params["collector.ntp.protocol-version"]
	if !ok {
		*ntpProtocolVersion = 4
	} else {
		ver, err := strconv.Atoi(nv)
		if err == nil {
			*ntpProtocolVersion = ver
		}
	}
	*ntpServerIsLocal = params["collector.ntp.server-is-local"] == "true"
	ttl, ok := params["collector.ntp.ip-ttl"]
	if !ok {
		*ntpIPTTL = 1
	} else {
	...
	// 3.46608s ~ 1.5s + PHI * (1 << maxPoll), where 1.5s is MAXDIST from ntp.org, it is 1.0 in RFC5905
	// max-distance option is used as-is without phi*(1<<poll)
	nmd, ok := params["collector.ntp.max-distance"]
	if !ok {
		*ntpMaxDistance = time.Duration(3466080) * time.Microsecond
	} else {
		md, err := time.ParseDuration(nmd)
		if err == nil {
			*ntpMaxDistance = md
		}
	}

	not, ok := params["collector.ntp.local-offset-tolerance"]
	if !ok {
		*ntpOffsetTolerance = time.Millisecond
	} else {
		nt, err := time.ParseDuration(not)
		if err == nil {
			*ntpOffsetTolerance = nt
		}
	}
}
