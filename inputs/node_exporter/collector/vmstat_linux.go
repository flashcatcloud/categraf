...
var (
	vmStatFields = new(string)
	// kingpin.Flag("collector.vmstat.fields", "Regexp of fields to return for vmstat collector.").Default("^(oom_kill|pgpg|pswp|pg.*fault).*").String()
)
...
func vmStatCollectorInit(params map[string]string) {
	vsf, ok := params["collector.vmstat.fields"]
	if !ok {
