...
var (
	collectorWifi = new(string)
	// kingpin.Flag("collector.wifi.fixtures", "test fixtures to use for wifi collector metrics").Default("").String()
)
...
func wifiCollectorInit(params map[string]string) {
	*collectorWifi = params["collector.wifi.fixtures"]
}
