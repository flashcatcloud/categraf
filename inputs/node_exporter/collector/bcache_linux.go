...
var (
	priorityStats = new(bool) // kingpin.Flag("collector.bcache.priorityStats", "Expose expensive priority stats.").Bool()
)
...
func bcacheCollectorInit(params map[string]string) {
	if val, ok := params["collector.bcache.priorityStats"]; ok {
		*priorityStats = val == "true"
	}
}
