...
var (
	collectorQdisc                 = new(string) // kingpin.Flag("collector.qdisc.fixtures", "test fixtures to use for qdisc collector end-to-end testing").Default("").String()
	collectorQdiscDeviceInclude    = new(string) // kingpin.Flag("collector.qdisc.device-include", "Regexp of qdisc devices to include (mutually exclusive to device-exclude).").String()
	oldCollectorQdiskDeviceInclude = new(string) // kingpin.Flag("collector.qdisk.device-include", "DEPRECATED: Use collector.qdisc.device-include").Hidden().String()
	collectorQdiscDeviceExclude    = new(string) // kingpin.Flag("collector.qdisc.device-exclude", "Regexp of qdisc devices to exclude (mutually exclusive to device-include).").String()
	oldCollectorQdiskDeviceExclude = new(string) // kingpin.Flag("collector.qdisk.device-exclude", "DEPRECATED: Use collector.qdisc.device-exclude").Hidden().String()
)
...
func NewQdiscStatCollector() (Collector, error) {
	if *oldCollectorQdiskDeviceInclude != "" {
		if *collectorQdiscDeviceInclude == "" {
			log.Println("W! --collector.qdisk.device-include is DEPRECATED and will be removed in 2.0.0, use --collector.qdisc.device-include")
	...
	if *oldCollectorQdiskDeviceExclude != "" {
		if *collectorQdiscDeviceExclude == "" {
			log.Println("W! --collector.qdisk.device-exclude is DEPRECATED and will be removed in 2.0.0, use --collector.qdisc.device-exclude")
...
func qdiscStatCollectorInit(params map[string]string) {
	*collectorQdisc = params["collector.qdisc.fixtures"]
	*collectorQdiscDeviceInclude = params["collector.qdisc.device-include"]
	*oldCollectorQdiskDeviceInclude = params["collector.qdisk.device-include"]
	*collectorQdiscDeviceExclude = params["collector.qdisc.device-exclude"]
	*oldCollectorQdiskDeviceExclude = params["collector.qdisk.device-exclude"]
}
