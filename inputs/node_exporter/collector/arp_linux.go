...
var (
	arpDeviceInclude = new(string) // = kingpin.Flag("collector.arp.device-include", "Regexp of arp devices to include (mutually exclusive to device-exclude).").String()
	arpDeviceExclude = new(string) // = kingpin.Flag("collector.arp.device-exclude", "Regexp of arp devices to exclude (mutually exclusive to device-include).").String()
	arpNetlink       = new(bool)   // = kingpin.Flag("collector.arp.netlink", "Use netlink to gather stats instead of /proc/net/arp.").Default("true").Bool()
)
...
func arpCollectorInit(params map[string]string) {
	*arpDeviceInclude = params["collector.arp.device-include"]
	*arpDeviceExclude = params["collector.arp.device-exclude"]
	netLink, ok := params["collector.arp.netlink"]
	if !ok {
		*arpNetlink = true
	} else {
		*arpNetlink = netLink == "true"
	}
}
