...
func diskstatsCollectorInit(params map[string]string) {
	exclude, ok := params["collector.diskstats.device-exclude"]
	if ok {
		diskstatsDeviceExcludeSet = true
		*diskstatsDeviceExclude = exclude
	} else {
		*diskstatsDeviceExclude = diskstatsDefaultIgnoredDevices
	}
	*oldDiskstatsDeviceExclude = params["collector.diskstats.ignored-devices"]

	*diskstatsDeviceInclude = params["collector.diskstats.device-include"]
}
