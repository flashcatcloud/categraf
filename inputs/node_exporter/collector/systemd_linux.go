...
var (
	...
	systemdUnitInclude    = new(string)
	/*
			kingpin.Flag("collector.systemd.unit-include", "Regexp of systemd units to include. Units must both match include and not match exclude to be included.").Default(".+").PreAction(func(c *kingpin.ParseContext) error {
			systemdUnitIncludeSet = true
			return nil
		}).String()
	*/
	oldSystemdUnitInclude = new(string) // kingpin.Flag("collector.systemd.unit-whitelist", "DEPRECATED: Use --collector.systemd.unit-include").Hidden().String()
	systemdUnitExcludeSet bool
	systemdUnitExclude    = new(string)
	/*
			kingpin.Flag("collector.systemd.unit-exclude", "Regexp of systemd units to exclude. Units must both match include and not match exclude to be included.").Default(".+\\.(automount|device|mount|scope|slice)").PreAction(func(c *kingpin.ParseContext) error {
			systemdUnitExcludeSet = true
			return nil
		}).String()
	*/
	oldSystemdUnitExclude = new(string)
	// kingpin.Flag("collector.systemd.unit-blacklist", "DEPRECATED: Use collector.systemd.unit-exclude").Hidden().String()
	systemdPrivate         = new(bool) // kingpin.Flag("collector.systemd.private", "Establish a private, direct connection to systemd without dbus (Strongly discouraged since it requires root. For testing purposes only).").Hidden().Bool()
	enableTaskMetrics      = new(bool) // kingpin.Flag("collector.systemd.enable-task-metrics", "Enables service unit tasks metrics unit_tasks_current and unit_tasks_max").Bool()
	enableRestartsMetrics  = new(bool) // kingpin.Flag("collector.systemd.enable-restarts-metrics", "Enables service unit metric service_restart_total").Bool()
	enableStartTimeMetrics = new(bool) // kingpin.Flag("collector.systemd.enable-start-time-metrics", "Enables service unit metric unit_start_time_seconds").Bool()
...
func systemdCollectorInit(params map[string]string) {
	csu, ok := params["collector.systemd.unit-include"]
	if !ok {
		*systemdUnitInclude = ".+"
	} else {
		systemdUnitIncludeSet = true
		*systemdUnitInclude = csu
	}

	*oldSystemdUnitInclude = params["collector.systemd.unit-whitelist"]

	// systemdUnitExcludeSet bool
	sue, ok := params["collector.systemd.unit-exclude"]
	if !ok {
		*systemdUnitExclude = ".+\\.(automount|device|mount|scope|slice)"
	} else {
		systemdUnitExcludeSet = true
		*systemdUnitExclude = sue
	}
	*oldSystemdUnitExclude = params["collector.systemd.unit-blacklist"]
	*systemdPrivate = params["collector.systemd.private"] == "true"
	*enableTaskMetrics = params["collector.systemd.enable-task-metrics"] == "true"
	*enableRestartsMetrics = params["collector.systemd.enable-restarts-metrics"] == "true"
	*enableStartTimeMetrics = params["collector.systemd.enable-start-time-metrics"] == "true"
}
