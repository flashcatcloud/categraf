...
var (
	perfCPUsFlag       = new(string)   // kingpin.Flag("collector.perf.cpus", "List of CPUs from which perf metrics should be collected").Default("").String()
	perfTracepointFlag = new([]string) // kingpin.Flag("collector.perf.tracepoint", "perf tracepoint that should be collected").Strings()
	perfNoHwProfiler   = new(bool)     // kingpin.Flag("collector.perf.disable-hardware-profilers", "disable perf hardware profilers").Default("false").Bool()
	perfHwProfilerFlag = new([]string) // kingpin.Flag("collector.perf.hardware-profilers", "perf hardware profilers that should be collected").Strings()
	perfNoSwProfiler   = new(bool)     // kingpin.Flag("collector.perf.disable-software-profilers", "disable perf software profilers").Default("false").Bool()
	perfSwProfilerFlag = new([]string) // kingpin.Flag("collector.perf.software-profilers", "perf software profilers that should be collected").Strings()
	perfNoCaProfiler   = new(bool)     // kingpin.Flag("collector.perf.disable-cache-profilers", "disable perf cache profilers").Default("false").Bool()
	perfCaProfilerFlag = new([]string) // kingpin.Flag("collector.perf.cache-profilers", "perf cache profilers that should be collected").Strings()
)
...
func perfCollectorInit(params map[string]string) {
	*perfCPUsFlag = params["collector.perf.cpus"]

	tp := strings.Split(params["collector.perf.tracepoint"], ",")
	for i := range tp {
		*perfTracepointFlag = append(*perfTracepointFlag, tp[i])
	}
	*perfNoHwProfiler = params["collector.perf.disable-hardware-profilers"] == "true"
	ps := strings.Split(params["collector.perf.hardware-profilers"], ",")
	for i := range ps {
		*perfHwProfilerFlag = append(*perfHwProfilerFlag, ps[i])
	}
	*perfNoSwProfiler = params["collector.perf.disable-software-profilers"] == "true"
	*perfNoCaProfiler = params["collector.perf.disable-cache-profilers"] == "true"
	pspf := strings.Split(params["collector.perf.software-profilers"], ",")
	for i := range pspf {
		*perfSwProfilerFlag = append(*perfSwProfilerFlag, pspf[i])
	}
	pcpf := strings.Split(params["collector.perf.cache-profilers"], ",")
	for i := range pcpf {
		*perfCaProfilerFlag = append(*perfCaProfilerFlag, pcpf[i])
	}
}
