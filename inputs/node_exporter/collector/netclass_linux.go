...
var (
	netclassIgnoredDevices = new(string) // kingpin.Flag("collector.netclass.ignored-devices", "Regexp of net devices to ignore for netclass collector.").Default("^$").String()
	netclassInvalidSpeed   = new(bool)   // kingpin.Flag("collector.netclass.ignore-invalid-speed", "Ignore devices where the speed is invalid. This will be the default behavior in 2.x.").Bool()
	netclassNetlink        = new(bool)   // kingpin.Flag("collector.netclass.netlink", "Use netlink to gather stats instead of /proc/net/dev.").Default("false").Bool()
)
...
func netClassCollectorInit(params map[string]string) {
	ncid, ok := params["collector.netclass.ignored-device"]
	if !ok {
		*netclassIgnoredDevices = "^$"
	} else {
		*netclassIgnoredDevices = ncid
	}
	*netclassInvalidSpeed = params["collector.netclass.ignore-invalid-speed"] == "true"
	*netclassNetlink = params["collector.netclass.netlink"] == "true"
	*netclassRTNLWithStats = params["collector.netclass_rtnl.with-stats"] == "true"
}
