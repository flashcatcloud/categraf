...
var (
	supervisordURL = new(string) // kingpin.Flag("collector.supervisord.url", "XML RPC endpoint.").Default("http://localhost:9001/RPC2").Envar("SUPERVISORD_URL").String()
	xrpc           *xmlrpc.Client
)
...
func supervisordCollectorInit(params map[string]string) {
	u, ok := params["collector.supervisord.url"]
	if !ok {
