...
var (
	sysctlInclude     = new([]string) // kingpin.Flag("collector.sysctl.include", "Select sysctl metrics to include").Strings()
	sysctlIncludeInfo = new([]string) // kingpin.Flag("collector.sysctl.include-info", "Select sysctl metrics to include as info metrics").Strings()
...
func sysctlCollectorInit(params map[string]string) {
	include := strings.Split(params["collector.sysctl.include"], ",")
	for i := range include {
		*sysctlInclude = append(*sysctlInclude, include[i])
	}

	includeInfo := strings.Split(params["collector.sysctl.include-info"], ",")
	for i := range includeInfo {
		*sysctlIncludeInfo = append(*sysctlIncludeInfo, includeInfo[i])
	}
}
