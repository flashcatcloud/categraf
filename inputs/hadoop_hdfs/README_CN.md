...
请参考：[hadoop-hdfs.toml](../../conf/example.input.jolokia_agent/hadoop-hdfs.toml)

具体步骤：
1. 将上述参考配置复制到您的 Categraf `conf/input.jolokia_agent/` 目录中。
...
由于实际上使用的是 Jolokia Agent，采集到的指标完全取决于配置文件中配置的 `metrics`。请在您的 Grafana 或夜莺监控大盘中直接使用 `jolokia_` 或者配置中指定的 `name_prefix` 作为前缀来查询指标。
