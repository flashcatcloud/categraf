...
// addXMLv2Counter adds a v2Counter array to a sample list, with the specified tags
func addXMLv2Counter(slist *types.SampleList, commonTags map[string]string, stats []v2Counter) {
	for _, c := range stats {
		...
		slist.PushSample("bind_counter", c.Name, c.Value, tags)
	}
}
...
func (b *Instance) readStatsXMLv2(addr *url.URL, slist *types.SampleList) error {
	var stats v2Root
	...
	// Opcodes
	tags["type"] = "opcode"
	addXMLv2Counter(slist, tags, stats.Statistics.Server.OpCodes)

	// Query RDATA types
	tags["type"] = "qtype"
	addXMLv2Counter(slist, tags, stats.Statistics.Server.RdTypes)

	// Nameserver stats
	tags["type"] = "nsstat"
	addXMLv2Counter(slist, tags, stats.Statistics.Server.NSStats)

	// Zone stats
	tags["type"] = "zonestat"
	addXMLv2Counter(slist, tags, stats.Statistics.Server.ZoneStats)

	// Socket statistics
	tags["type"] = "sockstat"
	addXMLv2Counter(slist, tags, stats.Statistics.Server.SockStats)
	...
	slist.PushSamples("bind_memory", fields, map[string]string{"url": addr.Host, "source": host, "port": port})
	...
	if b.GatherViews {
		for _, v := range stats.Statistics.Views {
			...
			// Query RDATA types
			tags["type"] = "qtype"
			addXMLv2Counter(slist, tags, v.RdTypes)

			// Resolver stats
			tags["type"] = "resstats"
			addXMLv2Counter(slist, tags, v.ResStats)
		}
	}
