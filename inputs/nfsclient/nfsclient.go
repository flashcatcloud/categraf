...
func (ins *Instance) Init() error {
	...
	if len(ins.IncludeMounts) > 0 {
		if config.Config.DebugMode {
			log.Println("D! Including these mount patterns:", ins.IncludeMounts)
		}
	} else {
	...
	if len(ins.ExcludeMounts) > 0 {
		if config.Config.DebugMode {
			log.Println("D! Excluding these mount patterns:", ins.ExcludeMounts)
		}
	} else {
		if config.Config.DebugMode {
	...
	if len(ins.IncludeOperations) > 0 {
		if config.Config.DebugMode {
			log.Println("D! Including these operations:", ins.IncludeOperations)
		}
	} else {
		if config.Config.DebugMode {
	...
	if len(ins.ExcludeOperations) > 0 {
		if config.Config.DebugMode {
			log.Println("D! Excluding these mount patterns:", ins.ExcludeOperations)
		}
	} else {
		if config.Config.DebugMode {
