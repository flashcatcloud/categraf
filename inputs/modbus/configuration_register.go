...
func (c *configurationOriginal) process() (map[byte]requestSet, error) {
	...
	coil, err := c.initRequests(c.Coils, maxQuantity, false, c.workarounds.ReadCoilsStartingAtZero)
	if err != nil {
		return nil, err
	}
	...
	discrete, err := c.initRequests(c.DiscreteInputs, maxQuantity, false, false)
	if err != nil {
		return nil, err
	}
	...
	holding, err := c.initRequests(c.HoldingRegisters, maxQuantity, true, false)
	if err != nil {
		return nil, err
	}
	...
	input, err := c.initRequests(c.InputRegisters, maxQuantity, true, false)
	if err != nil {
		return nil, err
	}
...
func (c *configurationOriginal) initRequests(fieldDefs []fieldDefinition, maxQuantity uint16, typed bool, enforceFromZero bool) ([]request, error) {
	...
	params := groupingParams{
		...
		optimization:    "none",
		enforceFromZero: enforceFromZero,
	}

	return groupFieldsToRequests(fields, params)
}
	...
	for _, item := range fieldDefs {
		...
		case "STRING":
			if len(item.Address) == 0 {
				return fmt.Errorf("address is empty in %q - %q", registerType, item.Name)
			}
			for i := 1; i < len(item.Address); i++ {
				if item.Address[i] != item.Address[i-1]+1 {
					return fmt.Errorf("string address must be consecutive in %q - %q", registerType, item.Name)
				}
			}
			continue
		case "BIT":
