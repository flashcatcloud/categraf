...
func splitMaxBatchSize(g request, maxBatchSize uint16) ([]request, error) {
	...
	for start := g.address; start < g.address+g.length; {
		...
		for _, f := range g.fields[idx:] {
			if f.length > maxBatchSize {
				return nil, fmt.Errorf("field %q (length %d) exceeds maximum batch size %d", f.name, f.length, maxBatchSize)
			}
		...
		// Prevent infinite loops if no fields could be added
		if len(current.fields) == 0 {
			return nil, fmt.Errorf("could not fit any fields into batch at address %d (max size %d)", start, maxBatchSize)
		}
...
func shrinkGroup(g request, maxBatchSize uint16) ([]request, error) {
	var requests []request
	var current request
...
func optimizeGroup(g request, maxBatchSize uint16) ([]request, error) {
	if len(g.fields) == 0 {
		return nil, nil
	}

	requests, err := shrinkGroup(g, maxBatchSize)
	if err != nil {
		return nil, err
	}
	...
	for i := 1; i < len(g.fields)-1; i++ {
		...
		curr1, err := shrinkGroup(request{fields: g.fields[:i]}, maxBatchSize)
		if err != nil {
			return nil, err
		}
		curr2, err := optimizeGroup(request{fields: g.fields[i:]}, maxBatchSize)
		if err != nil {
			return nil, err
		}
		current := append(curr1, curr2...)
...
func optimizeGroupWithinLimits(g request, params groupingParams) ([]request, error) {
	if len(g.fields) == 0 {
		return nil, nil
	}

	var requests []request
	if g.fields[0].length > params.maxBatchSize {
		return nil, fmt.Errorf("field %q (length %d) exceeds maximum batch size %d", g.fields[0].name, g.fields[0].length, params.maxBatchSize)
	}
	...
	for i := 1; i <= len(g.fields)-1; i++ {
		...
		if g.fields[i].address < g.fields[i-1].address+g.fields[i-1].length {
			slog.Warn("Request overlaps with next request",
				"prev_address", g.fields[i-1].address,
				"prev_length", g.fields[i-1].length,
				"next_address", g.fields[i].address,
			)
			holeSize = 0
		}
		...
		if g.fields[i].length > params.maxBatchSize {
			return nil, fmt.Errorf("field %q (length %d) exceeds maximum batch size %d", g.fields[i].name, g.fields[i].length, params.maxBatchSize)
		}
		currentRequest = request{
...
func groupFieldsToRequests(fields []field, params groupingParams) ([]request, error) {
	if len(fields) == 0 {
		return nil, nil
	}
	...
	if params.enforceFromZero {
		if groups[0].length+groups[0].address <= params.maxBatchSize {
			groups[0].length += groups[0].address
			groups[0].address = 0
		} else {
			slog.Warn("cannot enforce_read_from_zero because it would exceed max batch size", "address", groups[0].address, "length", groups[0].length, "maxBatchSize", params.maxBatchSize)
		}
	}
	...
	case "shrink":
		...
		for _, g := range groups {
			if len(g.fields) > 0 {
				res, err := shrinkGroup(g, params.maxBatchSize)
				if err != nil {
					return nil, err
				}
	...
	case "rearrange":
		...
		for _, g := range groups {
			if len(g.fields) > 0 {
				res, err := optimizeGroup(g, params.maxBatchSize)
				if err != nil {
					return nil, err
				}
				requests = append(requests, res...)
	...
	case "max_insert":
		...
		res, err := optimizeGroupWithinLimits(total, params)
		if err != nil {
			return nil, err
		}
		requests = res
	default:
		...
		for _, g := range groups {
			if len(g.fields) > 0 {
				res, err := splitMaxBatchSize(g, params.maxBatchSize)
				if err != nil {
					return nil, err
				}
				requests = append(requests, res...)
